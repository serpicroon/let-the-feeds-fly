...
async def init_db():
    async with aiosqlite.connect(settings.database) as db:
        ...
        await db.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_entries_hash ON entries(hash);")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_entries_feed_published_discovered ON entries(feed, published_at, discovered_at);")
...
async def upsert_entry(entry: Entry) -> None:
    ...
    async with aiosqlite.connect(settings.database) as db:
        async with db.execute("SELECT serialized, published_at FROM entries WHERE hash = ?", (entry.hash,)) as cursor:
            existing = await cursor.fetchone()
        ...
        if existing:
            ...
            if old_serialized != entry.serialized or old_published_at != entry.published_at:
                await db.execute("""
                    ...
                    WHERE hash = ?
                """, (entry.serialized, entry.published_at, entry.hash))
        else:
            await db.execute("""
